  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
        ...
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          {showSearch ? (
            ...
            <div className="flex items-center gap-2">
              ...
              {user ? (
                <>
                  <button onClick={() => navigate("/submit")} className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-semibold px-3 py-1.5 rounded-xl hover:opacity-90 flex items-center gap-1">
                  ...
                  <div className="relative group">
                    <div onClick={() => navigate("/library")} className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-xs font-bold cursor-pointer">
                      {user.email?.slice(0,1).toUpperCase()}
                    </div>
                  </div>
                  <button onClick={handleSignOut} className="text-xs text-gray-400 hover:text-gray-600 px-2 py-1 rounded-lg hover:bg-gray-100">
                    Sign out
                  </button>
                </>
              ) : (
                <>
                  <button onClick={() => navigate("/login")} className="text-xs font-semibold text-gray-600 px-3 py-1.5 rounded-xl border border-gray-200 hover:bg-gray-50">
                    Sign in
                  </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex sm:hidden z-30">
        {[
          { icon: "M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25", label: "Home", path: "/" },
          { icon: "M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z", label: "Search", path: "/search" },
          { icon: "M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z", label: "Library", path: "/library" },
          { icon: "M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z", label: "Profile", path: user ? "/library" : "/login" },
        ].map(({ icon, label, path }) => (
          <button key={label} onClick={() => navigate(path)} className={`flex-1 flex flex-col items-center justify-center py-3 gap-0.5 ${window.location.pathname === path ? "text-violet-600" : "text-gray-400"}`}>
