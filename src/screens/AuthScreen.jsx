    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-5 font-sans">
        <div className="bg-white border border-gray-100 rounded-2xl p-8 max-w-sm w-full text-center shadow-sm">
          ...
          <p className="text-sm text-gray-500 mb-5">We sent a confirmation link to <span className="font-semibold text-gray-700">{form.email}</span>. Click it to activate your account.</p>
          <button onClick={() => switchMode("login")} className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-bold py-3 rounded-xl text-sm">
            Back to login
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-5 font-sans">
        <div className="bg-white border border-gray-100 rounded-2xl p-8 max-w-sm w-full text-center shadow-sm">
          ...
          <p className="text-sm text-gray-500 mb-5">Check your inbox for a reset link.</p>
          <button onClick={() => switchMode("login")} className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-bold py-3 rounded-xl text-sm">
            Back to login
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      <div className="flex-1 flex flex-col justify-center px-5 py-8 max-w-sm mx-auto w-full">
        ...
        <div className="space-y-4">
          {mode !== "forgot" && (
            <>
              <button onClick={handleGoogle} className="w-full flex items-center justify-center gap-2.5 bg-white border border-gray-200 rounded-xl py-3 text-sm font-semibold text-gray-700 hover:bg-gray-50 active:scale-95 transition-all shadow-sm">
          ...
          <div className="space-y-3">
            {mode === "signup" && (
              <InputField label="Full name" placeholder="Awad" value={form.name} onChange={update("name")}
            ...
            <InputField label="Email address" type="email" placeholder="you@example.com" value={form.email} onChange={update("email")}
            ...
            {mode !== "forgot" && (
              <InputField label="Password" type="password" placeholder="Min 6 characters" value={form.password} onChange={update("password")}
            ...
            {mode === "signup" && (
              <InputField label="Confirm password" type="password" placeholder="Repeat your password" value={form.confirm} onChange={update("confirm")}
          ...
          {mode === "login" && (
            <div className="text-right -mt-1">
              <button onClick={() => switchMode("forgot")} className="text-xs text-violet-600 font-medium hover:underline">Forgot password?</button>
            </div>
          )}
          ...
          {mode === "signup" && (
            <label className="flex items-start gap-2.5 cursor-pointer">
              ...
              <span className="text-xs text-gray-500 leading-relaxed">
                I agree to the <span className="text-violet-600 font-medium">Terms of Service</span> and <span className="text-violet-600 font-medium">Privacy Policy</span>
              </span>
            </label>
          )}
          ...
          <p className="text-center text-xs text-gray-500 pt-1">
            {mode === "login" && <>Don't have an account?{" "}<button onClick={() => switchMode("signup")} className="text-violet-600 font-semibold hover:underline">Sign up free</button></>}
            {mode === "signup" && <>Already have an account?{" "}<button onClick={() => switchMode("login")} className="text-violet-600 font-semibold hover:underline">Sign in</button></>}
            {mode === "forgot" && <>Remember your password?{" "}<button onClick={() => switchMode("login")} className="text-violet-600 font-semibold hover:underline">Back to login</button></>}
          </p>
        </div>
      </div>

      <div className="text-center pb-6 px-4">
        <p className="text-[11px] text-gray-400">© 2025 Key Prompt · Operated by Awad</p>
      </div>
    </div>
  );
}